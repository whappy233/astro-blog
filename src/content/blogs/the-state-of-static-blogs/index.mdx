...
Hello! My name is carlos ([jktrn](https://github.com/jktrn) on GitHub), and I'm a fullstack web developer who has been fiddling with blogging platforms for a couple of years now. I run a blog at [carlos.dev](https://carlos.dev), where I write about cybersecurity and the capture-the-flag (CTF) scene.
...
- You can specify multiple post authors via frontmatter. If this post author's ID is found within the `Authors` collection, then it will render particular info from that author's frontmatter file, `[author-name].md` (e.g. avatar, link to profile). For example, the previous post (2024 Post) has two authors: "carlos" and "jktrn", where "carlos" is the only author with a custom avatar since "jktrn" is unregistered.
